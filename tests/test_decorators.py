...
def test_add(capsys):
    ...
    assert "add ok" in captured.out or "add ok" in captured.err
...
def test_raise_error(capsys):
    with pytest.raises(ValueError):
        raise_error()
    captured = capsys.readouterr()
    assert "raise_error error: Test error" in captured.out or "raise_error error: Test error" in captured.err
